...
export default function Gallery() {
  ...
  const { referenceImages, removeReferenceImage } = useImageContext();
  const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid');
  const [searchTerm, setSearchTerm] = useState('');
  const [sortBy, setSortBy] = useState<'newest' | 'oldest' | 'prompt'>('newest');

  const filteredImages = referenceImages
    .filter((img: any) =>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600">
            {t('gallery.imagesCount', { filtered: filteredImages.length, total: referenceImages.length })}
          </p>
        </div>
      ...
      {filteredImages.length === 0 ? (
        <div className="card text-center py-12">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-2">
            {referenceImages.length === 0 ? t('gallery.noImages') : t('gallery.noImagesFound')}
          </h3>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-gray-500">
                      {image.uploadedAt.toLocaleDateString()} at {image.uploadedAt.toLocaleTimeString()}
                    </span>
